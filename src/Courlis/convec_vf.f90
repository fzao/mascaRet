...
!    if (U(i) >= Umax) then
  ...
  if ((.not.ConsConv%Conv).OR.(Umax==0.)) then ! pas de convection
    ...
    if (ConsConv%Scheconv == 2) then     ! Forme non conservative - Caractéristiques en convection faible (convection de C)
    ...
    elseif (ConsConv%Scheconv == 3) then ! Forme conservative - Caractéristiques en convection faible (convection de A*C)
