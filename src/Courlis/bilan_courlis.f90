...
! Flux de depot / erosion par couche dans le bief
  Do k = 1, NbCouche
    FluxVaseCouche(k)  = FluxVaseCouche(k)  + QVaseCouche(k,1)  * (Absc(2) - Absc(1)) / W2
    FluxSableCouche(k) = FluxSableCouche(k) + QSableCouche(k,1) * (Absc(2) - Absc(1)) / W2
  Enddo
  Do i = 2, NbProfil -1
    Do k = 1, NbCouche
      FluxVaseCouche(k)  = FluxVaseCouche(k)  + QVaseCouche(k,i)  * (Absc(i+1) - Absc(i-1)) / W2
      FluxSableCouche(k) = FluxSableCouche(k) + QSableCouche(k,i) * (Absc(i+1) - Absc(i-1)) / W2
    Enddo
  Enddo
  Do k = 1, NbCouche
    FluxVaseCouche(k)  = FluxVaseCouche(k)  +  &
                                   QVaseCouche(k,NbProfil)  * (Absc(NbProfil) - Absc(NbProfil-1)) / W2
    FluxSableCouche(k) = FluxSableCouche(k) +  &
                                   QSableCouche(k,NbProfil) * (Absc(NbProfil) - Absc(NbProfil-1)) / W2
  Enddo
  ...
  Do i = 1, NbProfil
    Do k = 1, NbCouche
      DepotCumulCouche(k,i) = DepotCumulCouche(k,i)    &
                                                - (QVaseCouche(k,i) + QSableCouche(k,i)) * Dt / 1000._DOUBLE
    Enddo
  Enddo

! Masse de sediment deposee par couche dans le bief depuis T=0
  Do k = 1, NbCouche
    MasseVase%DepotCouche(k)  = MasseVase%DepotCouche(k)  - FluxVaseCouche(k)  * Dt
    MasseSable%DepotCouche(k) = MasseSable%DepotCouche(k) - FluxSableCouche(k) * Dt
  Enddo

! Masse de sédiment dans l'eau a l'instant t
  MasseVase%Eau  = CVase(1)  * Sm(1) * (Absc(2) - Absc(1)) / W2
  MasseSable%Eau = CSable(1) * Sm(1) * (Absc(2) - Absc(1)) / W2
  Do i = 2, NbProfil-1
    MasseVase%Eau  = MasseVase%Eau  + CVase(i)  * Sm(i) * (Absc(i+1) - Absc(i-1)) / W2
    MasseSable%Eau = MasseSable%Eau + CSable(i) * Sm(i) * (Absc(i+1) - Absc(i-1)) / W2
  Enddo
  MasseVase%Eau  = MasseVase%Eau  + CVase(NbProfil)  * Sm(NbProfil) * (Absc(NbProfil) - Absc(NbProfil-1)) / W2
  MasseSable%Eau = MasseSable%Eau + CSable(NbProfil) * Sm(NbProfil) * (Absc(NbProfil) - Absc(NbProfil-1)) / W2
...
! Erreur relative a l'instant t
  ...
  Else
    Err = MasseVase%Eau - MasseVaseEauPrec -  &
                        (FluxVase%Entrant + FluxVase%Apport - FluxVase%Sortant + FluxVase%Depot) * Dt
  ...
  Else
    Err = MasseSable%Eau - MasseSableEauPrec -  &
                         (FluxSable%Entrant + FluxSable%Apport - FluxSable%Sortant + FluxSable%Depot) * Dt
...
! Calcul
  Do i = 1, NbProfil
    ...
    DeltaSurfaceSed(i) = DeltaH(1,i) * (ProfilCourlis(i)%X(2) - ProfilCourlis(i)%X(1)) / W2
    Do j = 2, NPt-1
      DeltaSurfaceSed(i) = DeltaSurfaceSed(i) +  &
                               DeltaH(j,i) * (ProfilCourlis(i)%X(j+1) - ProfilCourlis(i)%X(j-1)) / W2
    Enddo
    DeltaSurfaceSed(i) = DeltaSurfaceSed(i) +  &
                             DeltaH(NPt,i) * (ProfilCourlis(i)%X(NPt) - ProfilCourlis(i)%X(NPt-1)) / W2

    If (i == 1) Then
      VolSedDepot = VolSedDepot + DeltaSurfaceSed(i) * (Absc(i+1) - Absc(i)) / W2
    Elseif (i == NbProfil) Then
      VolSedDepot = VolSedDepot + DeltaSurfaceSed(i) * (Absc(i) - Absc(i-1)) / W2
    Else
      VolSedDepot = VolSedDepot + DeltaSurfaceSed(i) * (Absc(i+1) - Absc(i-1)) / W2
    Endif
