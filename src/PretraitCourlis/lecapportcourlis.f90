...
!=========================================================================
  Do iApport = 1, NbApports
	...
	! Test sur les numeros de loi
	If (ApportVase(iApport)%NumeroLoi  <= 0)		Then
	  write(txt,'(a,i3,a)') 'Numero de loi de l''apport ',iApport,' de Vase ou Sable'
	  Erreur%Numero = 408
	  Erreur%ft   = err_408
	  Erreur%ft_c = err_408c
	...
	If (ImpressionApport) Then
	  write(UniteList,1030) 'Nom      ', &
			ApportVase(iApport)%Nom		      , ApportSable(iApport)%Nom
	  write(UniteList,1040) 'NumeroLoi', &
			ApportVase(iApport)%NumeroLoi     , ApportSable(iApport)%NumeroLoi
	  write(UniteList,1040) 'Type     ', &
			ApportVase(iApport)%Type          , ApportSable(iApport)%Type
	  write(UniteList,1040) 'SectionAm', &
			ApportVase(iApport)%SectionAm     , ApportSable(iApport)%SectionAm
	  write(UniteList,1040) 'SectionAv', &
			ApportVase(iApport)%SectionAv     , ApportSable(iApport)%SectionAv
	  write(UniteList,1050) 'Q_source ', &
			ApportVase(iApport)%debit_source  , ApportSable(iApport)%debit_source
	  write(UniteList,1040) 'No. Trac.', &
			ApportVase(iApport)%Numero_traceur, ApportSable(iApport)%Numero_traceur	
	End if
...
!=========================================================================
  ...
  CL_Sable%Nom = "Condition limite amont en sable"
