...
use M_PARAMETRE_C      ! Parametres numériques
...
!chb-21.10.03    Profil(iProf)%LimiteMin(2)/=NbPointsCourlis-1) then
  If ((Profil(iProf)%LimiteMin(1)/=2).and.(Profil(iProf)%LimiteMin(2)/=NbPoints-1)) Then
    Erreur%Numero  = 405
    Erreur%ft  = err_405
...
!chb-21.10.03  if(abs(Profil(iProf)%X(iPoint)-ProfilCourlis(iProf)%X(iPoint)) > EPS2) then
    If (abs(Profil(iProf)%X(iPoint+1)-ProfilCourlis(iProf)%X(iPoint)) > EPS2) Then
    Erreur%Numero  = 403
    Erreur%ft  = err_403
...
!chb-21.10.03  if(abs(Profil(iProf)%Y(iPoint)-ProfilCourlis(iProf)%Z(1,iPoint)) > EPS2) then
    If (abs(Profil(iProf)%Y(iPoint+1)-ProfilCourlis(iProf)%Z(1,iPoint)) > EPS2) Then
    Erreur%Numero  = 404
    Erreur%ft  = err_404
